...
Phiên bản `2026.07.19.2` bổ sung hồ sơ học sinh tự cập nhật, ảnh đại diện riêng, nộp bài bằng nội dung và tệp đính kèm, chọn hạn bài tập theo buổi học kế tiếp của đúng môn, cùng bộ lọc sắp xếp bài tập. Giáo viên có thể sửa thông tin thương hiệu, đổi ảnh đại diện và logo ngay trong **Cài đặt**; các thay đổi được đồng bộ để dùng chung trên máy tính, điện thoại và cổng học sinh.

## Kích hoạt cổng học sinh

1. Chạy toàn bộ `supabase/student_portal.sql` trong Supabase SQL Editor.
2. Chạy tiếp `supabase/student_features.sql` để tạo hồ sơ, bài nộp và kho tệp riêng tư `student-work`.
3. Triển khai lại Edge Function trong `supabase/functions/student-portal`.
4. Mở mục **Tài khoản HS** trên trang quản trị để tạo tài khoản và cấp mật khẩu tạm.

Tệp học sinh tải lên không được công khai. Mỗi học sinh chỉ có thể tạo đường dẫn tải lên cho đúng thư mục của mình qua Edge Function; giáo viên xem tệp bằng liên kết ký có thời hạn. Ảnh đại diện và tệp bài làm tối đa 15 MB mỗi tệp, mỗi bài nhận tối đa 5 tệp.
